...
<template>
	<RouterLink v-bind:to="route" class="flex flex-col rounded shadow p-6 bg-light-grey
		dark:bg-slate-700">
		...
		<ul class="flex-1 pb-2">
			<li v-for="film in character.films" class="text-gray-800 dark:text-white" :key="film">{{ film }}</li>
		</ul>
		...
		<div class="flex gap-2 items-center">
			...
			<p class="text-blueish-grey dark:text-white">{{ new Intl.DateTimeFormat('en-US', {
				dateStyle: 'medium',
