  ...
  <section>
    <div class="flex flex-col md:flex-row pb-6 gap-4 md:justify-between">
      <input class="p-1 px-2 border rounded" v-model="searchValue"
        @input="event => updateFilter('search', (event.target as HTMLInputElement).value ?? '')">
      <select v-model="gender" class="border rounded p-1"
        @select="event => updateFilter('gender', (event.target as HTMLInputElement).value ?? '')">
      ...
      <select v-model="hairColor" class="border rounded p-1"
        @select="event => updateFilter('hairColor', (event.target as HTMLInputElement).value ?? '')">
